...
function buildGenerationPreviewState(id, bestResult, analysis){
  ...
  const sourceGroups = bestResult?.groups || state?.lotteries?.[id]?.generatedGroups?.groups || {};
  const sourceValues = Object.values(sourceGroups).filter(v => Array.isArray(v));
  const fallbackMainGroups = orderNames.map((name, idx) => {
    const direct = Array.isArray(sourceGroups?.[name]) ? sourceGroups[name] : null;
    const byOrder = Array.isArray(sourceValues[idx]) ? sourceValues[idx] : [];
    const picked = direct && direct.length ? direct : byOrder;
    return (picked || []).map(n => String(n).padStart(2,'0'));
  });
  ...
  let groupRows = (metrics.groupDetails || []).map((detail, idx)=>{
  ...
  if(!groupRows.length || groupRows.every(g => !g.nums.length)){
    groupRows = fallbackMainGroups.map((nums, idx)=>( {
      idx: idx + 1,
      nums: (nums || []).map(n=>String(n).padStart(2,'0')),
      score: 0,
      status: (bestResult?.canNotify === false ? '需留意' : '可用'),
      reason: (bestResult?.canNotify === false ? '本輪建議再生' : '分散正常，可保留'),
      riskyCount: 0,
      pairHits: 0,
      tripleHits: 0
    }));
...
${preview.worst.reason}` : '尚無';
  if($('generationPreviewGroups')) $('generationPreviewGroups').innerHTML = preview.groups.map(g=>`<div style="padding:10px 12px;border:1px solid rgba(255,255,255,.09);border-radius:14px;margin-top:8px;">`+
    `<b>第${g.idx}組【${escapeHtml(g.status)}】</b> ${escapeHtml((g.nums && g.nums.length ? g.nums : ['未帶入號碼']).join('、'))}<br>`+
