...
(() => {
  ...
  const state = {
    ...
    auth: {
      token: localStorage.getItem('lottery_member_token') || '',
      user: null
    }
  };
...
function cleanupNumbers(arr, maxNum){
  ...
  function setAuthReady(user){
    state.auth.user = user || null;
    document.body.classList.remove('auth-pending');
    document.body.classList.add('auth-ready');
    const overlay = $('authOverlay');
    if(overlay) overlay.style.display = 'none';
    const nameText = $('memberNameText');
    if(nameText && user){
      const expiredText = user?.expiresAt ? `｜到期 ${String(user.expiresAt).slice(0,10)}` : '';
      nameText.textContent = `${user.displayName || user.username}｜${user.role === 'admin' ? '管理員' : '會員'}${expiredText}`;
    }
    const adminPanel = $('adminPanel');
    if(adminPanel) adminPanel.classList.toggle('show', user?.role === 'admin');
    if(user?.role === 'admin') loadAdminDashboard().catch(err=>showMiniNotice(err.message || '載入會員後台失敗','warn'));
  }

  function resetAuthState(){
    state.auth.user = null;
    state.auth.token = '';
    localStorage.removeItem('lottery_member_token');
  ...
  async function verifyMemberToken(){
    ...
    try{
      const data = await fetchJson(apiPathCandidates('/api/auth/me'), { allowUnauthorized: true });
      if(!data?.ok || !data?.user) throw new Error(data?.message || '登入驗證失敗');
      setAuthReady(data.user);
      return true;
    }catch(err){
      resetAuthState();
      return false;
  ...
  async function loginMember(){
    ...
    if(!username || !password){
      setAuthError('請先輸入帳號與密碼');
      return false;
    }
    setAuthError('');
    const btn = $('loginBtn');
    const oldText = btn?.textContent || '登入系統';
    if(btn){ btn.disabled = true; btn.textContent = '登入中...'; }
    try{
      const result = await postJsonApi('/api/auth/login', { username, password }, { allowUnauthorized: true });
      if(!result?.ok || !result?.token) throw new Error(result?.message || '登入失敗');
      state.auth.token = result.token;
      localStorage.setItem('lottery_member_token', result.token);
      setAuthReady(result.user || null);
      setAuthError('');
      if(result?.isDefaultAdmin){
        showMiniNotice('已使用預設管理員帳號登入，建議儘快建立專屬會員並修改密碼', 'warn');
      }else{
        showMiniNotice('會員登入成功', 'ok');
      }
      return true;
    }catch(err){
  ...
  async function logoutMemberClient(){
    try{ await postJsonApi('/api/auth/logout', {}, { allowUnauthorized: true }); }catch(_){ }
    resetAuthState();
    showMiniNotice('已登出', 'info');
  }

  function adminPlanDays(plan, manualDays){
    if(plan === 'quarter') return 90;
    if(plan === 'year') return 365;
    if(plan === 'custom') return Math.max(1, Number(manualDays || 0));
    if(plan === 'permanent') return 36500;
    return 30;
  }
  ...
  async function loadAdminDashboard(){
    ...
    const [membersRes, keysRes] = await Promise.all([
      fetchJson(apiPathCandidates('/api/admin/members')),
      fetchJson(apiPathCandidates('/api/admin/access-keys'))
    ]);
    renderAdminMembers(Array.isArray(membersRes?.members) ? membersRes.members : []);
    renderAdminKeys(Array.isArray(keysRes?.keys) ? keysRes.keys : []);
  }

  function renderAdminMembers(members){
    const tbody = $('adminMembersTbody');
    if(!tbody) return;
    tbody.innerHTML = '';
    if(!members.length){
      tbody.innerHTML = '<tr><td colspan="4" class="small">尚無會員資料</td></tr>';
      return;
    }
    members.forEach(member => {
      const tr = document.createElement('tr');
      const statusText = member.status === 'disabled' ? '停用' : (member.isExpired ? '已到期' : '正常');
      tr.innerHTML = `
        <td>
          <div><b>${escapeHtml(member.displayName || member.username)}</b></div>
          <div class="small mono">${escapeHtml(member.username)}<br>ID: ${escapeHtml(member.id || '')}</div>
        </td>
        <td>
          <div><span class="planTag">${escapeHtml(member.planType || 'month')}</span></div>
          <div class="small">到期：${escapeHtml(fmtDateTime(member.expiresAt))}</div>
        </td>
        <td>${escapeHtml(statusText)}</td>
        <td>
          <div class="adminActions">
            <button class="miniBtn" data-action="extend" data-id="${escapeHtml(member.id)}">續期</button>
            <button class="miniBtn" data-action="toggle" data-id="${escapeHtml(member.id)}">${member.status === 'disabled' ? '啟用' : '停用'}</button>
            <button class="miniBtn green" data-action="password" data-id="${escapeHtml(member.id)}">改密碼</button>
          </div>
        </td>`;
      tbody.appendChild(tr);
    });
    tbody.querySelectorAll('button[data-action]').forEach(btn => btn.onclick = handleAdminMemberAction);
  }

  function renderAdminKeys(keys){
    const tbody = $('adminKeysTbody');
    if(!tbody) return;
    tbody.innerHTML = '';
    if(!keys.length){
      tbody.innerHTML = '<tr><td colspan="4" class="small">尚無金鑰資料</td></tr>';
      return;
    }
    keys.slice(0, 200).forEach(key => {
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td class="mono">${escapeHtml(key.code)}</td>
        <td>${escapeHtml(key.planType)} / ${escapeHtml(String(key.durationDays || 0))}天</td>
        <td>${escapeHtml(key.status)}</td>
        <td>${escapeHtml(key.usedBy || '—')}</td>`;
      tbody.appendChild(tr);
    });
  }

  async function createMemberFromAdmin(){
    const username = ($('adminMemberUsername')?.value || '').trim();
    const displayName = ($('adminMemberDisplayName')?.value || '').trim();
    const password = ($('adminMemberPassword')?.value || '').trim();
    const planType = $('adminMemberPlan')?.value || 'month';
    const durationDays = adminPlanDays(planType, $('adminMemberDays')?.value || '30');
    const note = ($('adminMemberNote')?.value || '').trim();
    if(!username || !password){ showMiniNotice('請輸入會員帳號與密碼', 'warn'); return; }
    await postJsonApi('/api/admin/members', { username, displayName, password, planType, durationDays, note });
    showMiniNotice('會員建立成功', 'ok');
    ['adminMemberUsername','adminMemberDisplayName','adminMemberPassword','adminMemberNote'].forEach(id=>{ const el=$(id); if(el) el.value=''; });
    await loadAdminDashboard();
  }

  async function generateKeysFromAdmin(){
    const planType = $('adminKeyPlan')?.value || 'month';
    const count = Number($('adminKeyCount')?.value || 1);
    const durationDays = adminPlanDays(planType, $('adminKeyDays')?.value || '30');
    const prefix = ($('adminKeyPrefix')?.value || 'SVN').trim() || 'SVN';
    const result = await postJsonApi('/api/admin/access-keys/generate', { planType, count, durationDays, prefix });
    showMiniNotice(`已生成 ${result?.keys?.length || 0} 組金鑰`, 'ok');
    await loadAdminDashboard();
  }

  async function redeemKeyFromAdmin(){
    const code = ($('adminRedeemCode')?.value || '').trim();
    const targetUserId = ($('adminRedeemUserId')?.value || '').trim();
    if(!code || !targetUserId){ showMiniNotice('請輸入金鑰與會員 ID', 'warn'); return; }
    await postJsonApi('/api/admin/access-keys/redeem', { code, targetUserId });
    showMiniNotice('金鑰已套用到會員', 'ok');
    await loadAdminDashboard();
  }

  async function handleAdminMemberAction(ev){
    ...
    try{
      if(action === 'extend'){
        const planType = prompt('輸入續期方案：month / quarter / year / custom', 'month') || 'month';
        const days = planType === 'custom' ? Number(prompt('請輸入自訂天數', '30') || '30') : adminPlanDays(planType, 30);
        await postJsonApi(`/api/admin/members/${id}/extend`, { planType, durationDays: days });
        showMiniNotice('會員已續期', 'ok');
      }else if(action === 'toggle'){
        const isDisable = btn.textContent.includes('停用');
        await postJsonApi(`/api/admin/members/${id}/update`, { status: isDisable ? 'disabled' : 'active' });
        showMiniNotice(isDisable ? '會員已停用' : '會員已啟用', 'ok');
      }else if(action === 'password'){
        const password = prompt('請輸入新密碼');
        if(!password) return;
        await postJsonApi(`/api/admin/members/${id}/update`, { password });
        showMiniNotice('會員密碼已更新', 'ok');
      }
      await loadAdminDashboard();
    }catch(err){
      showMiniNotice(err.message || '會員操作失敗', 'warn');
    }
  }
  ...
  async function postJsonApi(path, payload, options = {}){
    let lastErr = null;
    for(const target of apiPathCandidates(path)){
      try{
        ...
        if(res.status === 401 && !options.allowUnauthorized){ resetAuthState(); throw new Error(json.message || '請先登入會員'); }
  ...
  async function fetchJson(url, options = {}){
    ...
    for(const target of urls){
      if(!target) continue;
      try{
        const res = await fetch(target, { cache: "no-store", headers: getAuthHeaders() });
        const text = await res.text();
        if(res.status === 401 && !options.allowUnauthorized){ resetAuthState(); throw new Error('請先登入會員'); }
...
function clearBroadcastForm(){
  ...
  async function init(){
    $('loginHint').textContent = '預設管理員帳號：admin｜預設密碼：admin12345';
    $('loginBtn')?.addEventListener('click', loginMember);
    $('loginPassword')?.addEventListener('keydown', (e)=>{ if(e.key === 'Enter') loginMember(); });
    $('loginUsername')?.addEventListener('keydown', (e)=>{ if(e.key === 'Enter') loginMember(); });
    $('logoutBtn')?.addEventListener('click', logoutMemberClient);
    $('adminCreateMemberBtn')?.addEventListener('click', ()=>createMemberFromAdmin().catch(err=>showMiniNotice(err.message || '建立會員失敗','warn')));
    $('adminReloadMembersBtn')?.addEventListener('click', ()=>loadAdminDashboard().catch(err=>showMiniNotice(err.message || '讀取會員失敗','warn')));
    $('adminGenerateKeysBtn')?.addEventListener('click', ()=>generateKeysFromAdmin().catch(err=>showMiniNotice(err.message || '生成金鑰失敗','warn')));
    $('adminReloadKeysBtn')?.addEventListener('click', ()=>loadAdminDashboard().catch(err=>showMiniNotice(err.message || '讀取金鑰失敗','warn')));
    $('adminRedeemKeyBtn')?.addEventListener('click', ()=>redeemKeyFromAdmin().catch(err=>showMiniNotice(err.message || '套用金鑰失敗','warn')));
    $('loginPassword')?.addEventListener('keydown', (e)=>{ if(e.key === 'Enter') loginMember(); });
    $('loginUsername')?.addEventListener('keydown', (e)=>{ if(e.key === 'Enter') loginMember(); });

    const authed = await verifyMemberToken();
    if(!authed){
      setTaipeiClock();
      return;
    }
    ...
    if(localStorage.getItem('lottery_member_default_notice') === '1'){
      localStorage.removeItem('lottery_member_default_notice');
      showMiniNotice('目前使用預設管理員帳號，建議後續改密碼', 'warn');
    }
    loadTaskQueue();
    ensureTaskCenterUI();
